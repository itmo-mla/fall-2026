...
def main():
    ...
    for i in range(10000):
        margin = lin_model.predict_margin(
        lin_model._add_ones(X_train), y_train
        )
        neg_mag_idx = (np.ones_like(margin) == 1)[:, 0]
        mini_batch_x, mini_batch_y = fetch_batch(X_train[neg_mag_idx], y_train[neg_mag_idx], batch_size)
    ...
    x = list(range(1000))
    y = [metric_train_list[int(i)] for i in np.linspace(0, len(metric_train_list) - 1, 1000)]
    c = ['train'] * 1000

    x += x
    y += [metric_test_list[int(i)] for i in np.linspace(0, len(metric_test_list) - 1, 1000)]
    c += ['test'] * 1000
